...
const PalletQuoteCalculator = ({ isEmbedded = false }: { isEmbedded?: boolean }) => {
  ...
  if (!started) {
    return (
      <div 
        className={`relative w-full h-full flex flex-col items-center justify-center p-8 overflow-hidden rounded-sm transition-all duration-500 group bg-clip-padding ${isEmbedded ? 'border-4 border-neutral-700 hover:border-[#ffea05] shadow-[0_30px_60px_-10px_rgba(0,0,0,0.8)] bg-[#0A0A0A] hover:shadow-[0_0_40px_rgba(255,234,5,0.1)]' : ''}`}
        style={{
      ...
      >
        {/* Dark Overlay */}
        <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] transition-all duration-500 group-hover:bg-black/50"></div>
  ...
  return (
    <div 
        className={`flex flex-col ${isEmbedded ? 'bg-[#121212] border border-white/20 ring-1 ring-white/10 rounded-sm shadow-[0_30px_70px_-10px_rgba(0,0,0,0.9)] relative overflow-hidden transition-[height] duration-500 ease-in-out' : ''}`}
    ...
    >
       ...
       {/* Footer */}
       <div className="p-3 bg-[#1a1a1a] border-t border-white/5 flex gap-2 relative z-10">
          {step > 1 && (
...
const Navbar = () => {
    return (
        <nav className="fixed top-0 w-full z-50 bg-[#111]/90 backdrop-blur-md border-b border-white/10">
            <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
                <div className="flex items-center gap-2">
                     {/* Placeholder Logo */}
                    <div className="w-10 h-10 bg-[#FFEA05] rounded-sm flex items-center justify-center">
                        <Box className="text-black stroke-[2.5px]" />
                    </div>
                    <span className="font-bold text-xl tracking-tighter text-white">SUN PAC<span className="text-[#FFEA05]">.</span></span>
                </div>
                
                <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-400">
                    <a href="#" className="hover:text-[#FFEA05] transition-colors">Capabilities</a>
                    <a href="#" className="hover:text-[#FFEA05] transition-colors">Sustainability</a>
                    <a href="#" className="hover:text-[#FFEA05] transition-colors">Tracking</a>
                    <button className="bg-[#FFEA05] text-black px-6 py-2 rounded-sm font-bold text-xs uppercase tracking-wider hover:bg-white transition-all">
                        Get Quote
                    </button>
                </div>
            </div>
        ...
        <section className="relative pt-32 pb-24 px-6 overflow-hidden min-h-[90vh] flex items-center">
             ...
             <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center relative z-10 w-full">
                 <div className="space-y-8">
                     <div className="inline-flex items-center gap-2 border border-[#FFEA05]/30 bg-[#FFEA05]/5 text-[#FFEA05] px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-widest">
                     ...
                     <h1 className="text-5xl md:text-7xl font-bold text-white leading-none tracking-tight">
                         Engineered <br/>
                         <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FFEA05] to-orange-400">
                            For Speed.
                         </span>
                     </h1>
                     
                     <p className="text-gray-400 text-lg max-w-lg leading-relaxed border-l-2 border-[#FFEA05]/20 pl-6">
                         From rough lumber to precision components. We handle high-volume notching, ripping, and resawing with <span className="text-white font-bold">0.01mm accuracy</span>.
                     </p>
                 </div>

                 {/* EMBEDDED CALCULATOR */}
                 <div className="relative animate-in slide-in-from-bottom-8 fade-in duration-700 delay-100">
                    <PalletQuoteCalculator isEmbedded={true} />
                 </div>
             </div>
        </section>
    );
...
const ProcessTimeline = () => {
    const steps = [
        { id: "01", title: "Sourcing", desc: "Certified SPF Lumber", icon: <Leaf size={20} /> },
        { id: "02", title: "Cutting", desc: "Automated Precision Saws", icon: <Scissors size={20} /> },
        { id: "03", title: "Assembly", desc: "High-Speed Nailing", icon: <Hammer size={20} /> },
        { id: "04", title: "Logistics", desc: "Just-In-Time Delivery", icon: <Truck size={20} /> },
    ];
        ...
        <section className="py-24 bg-[#151515] border-t border-white/5">
            <div className="max-w-7xl mx-auto px-6">
                <div className="grid md:grid-cols-4 gap-8 relative">
                    {/* Connector Line */}
                    <div className="hidden md:block absolute top-[2.25rem] left-0 w-full h-[1px] bg-gradient-to-r from-[#FFEA05]/0 via-[#FFEA05]/20 to-[#FFEA05]/0 z-0"></div>
                    
                    {steps.map((step) => (
                        <div key={step.id} className="relative z-10 group">
                            <div className="w-16 h-16 bg-[#202020] border border-white/10 rounded-sm flex items-center justify-center text-gray-500 mb-6 group-hover:border-[#FFEA05] group-hover:text-[#FFEA05] transition-all duration-300 shadow-xl group-hover:shadow-[0_0_20px_rgba(255,234,5,0.1)]">
                                {step.icon}
                            </div>
                            <span className="text-[#FFEA05] font-mono text-xs opacity-50 mb-2 block">{step.id}</span>
    ...
    return (
        <section className="py-24 px-6 bg-[#111]">
             <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
                 <div>
                     <h2 className="text-4xl font-bold text-white mb-6">Efficiency by Design.</h2>
                     <p className="text-gray-400 leading-relaxed mb-8">
                         We don't just build pallets; we engineer them. By customizing the design to your exact load requirements, we reduce wood volume without compromising strength.
                     </p>
                     
                     <div className="space-y-6">
                         {[
                             { title: "Cost Reduction", desc: "Reduce wood waste & shipping weight." },
                             { title: "Damage Control", desc: "Custom load path analysis prevents loss." },
                             { title: "Scale Ready", desc: "Production capacity for 500+ units/day." }
                         ].map((item, i) => (
                             <div key={i} className="flex gap-4">
                                 <div className="w-1 h-full bg-[#FFEA05] rounded-full"></div>
                                 <div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                     <div className="bg-[#1a1a1a] p-6 rounded-sm border border-white/5 h-48 flex flex-col justify-end group hover:border-[#FFEA05]/50 transition-colors">
                         <BarChart3 className="text-gray-600 group-hover:text-[#FFEA05] mb-auto" size={24} />
                         <span className="text-3xl font-bold text-white font-mono">-15%</span>
                         <span className="text-xs text-gray-500 uppercase tracking-wide">Weight Reduced</span>
                     </div>
                     <div className="bg-[#1a1a1a] p-6 rounded-sm border border-white/5 h-48 flex flex-col justify-end translate-y-8 group hover:border-[#FFEA05]/50 transition-colors">
                         <ShieldCheck className="text-gray-600 group-hover:text-[#FFEA05] mb-auto" size={24} />
